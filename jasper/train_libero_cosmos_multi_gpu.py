...
def main():
    ...
    prompt_embeds_path = (
        "/home/ubuntu/workspace/LIBERO/libero/datasets/libero_90/prompt_embeds.pt"
    )
    latent_dir = "/home/ubuntu/workspace/LIBERO/libero/datasets/libero_90_vae_latents"
    ...
    for step in range(start_step, max_steps):
        ...
        head = batch["agentview_latent"].to(device, non_blocking=True)
        wrist = batch["wrist_latent"].to(device, non_blocking=True)
        images = torch.stack([head, wrist], dim=1)
        prompt_embs = batch["prompt_embeds"].to(device, non_blocking=True)
        ...
        if is_main and (step + 1) % log_every == 0:
            ...
            if step + 1 == log_every:
                ...
                print(
                    f"GPU {local_rank} peak memory: {mem_alloc:.1f}GB allocated, {mem_reserved:.1f}GB reserved"
                )
